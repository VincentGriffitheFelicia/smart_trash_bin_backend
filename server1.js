...
app.post('/api/bin', validateData, async (req, res) => {
    ...
    try {
        ...
        // Extract necessary data from bin
        const { Address, Geolocation, Height } = binData

        // Calculate fill level percentage
        const fillLevelPercentage = calculateFillLevel(Height, Distance)
        ...
        await db.collection('fill_levels').add({
            ...
            Token, // Store the token as part of the data
            Address,
            Geolocation,
            Height,
        })
        ...
        res.status(200).json({
            ...
            binDetails: { Address, Geolocation, Height },
            fillLevelPercentage,
        })
    } catch (err) {
